...
import { LandmarksNavList } from '@/features/heritageNavigation';
import { LanguageSwitch } from '@/features/languageSwitch';
import type { MobileMenuCloseReason } from '@/features/mobileMenu';
		...
		>
			...
			>
				<div className="flex h-16 shrink-0 items-center justify-between border-b border-theme-soft px-4">
					<span
						id={titleId}
						className="font-body text-xs uppercase tracking-[0.2em] text-theme-accent"
					>
				...
				<div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-2 pb-8 pt-4">
					<p className="mb-2 px-2 font-body text-[10px] uppercase tracking-[0.25em] text-theme-accent-soft">
						{tNav('landmarks')}
					</p>
					<div className="overflow-hidden rounded-xl border border-theme-soft bg-theme-surface-soft">
						<LandmarksNavList
							...
							isLoading={isLoading}
							locale={locale}
							onItemNavigate={() => onClose('item_navigate')}
							presentation="list"
						/>
					</div>
					<div className="my-6 h-px bg-gradient-theme-accent-soft" />
					<p className="mb-3 px-2 font-body text-[10px] uppercase tracking-[0.25em] text-theme-accent-soft">
						{tCommon('language_label')}
					</p>
					<div className="px-2 [&_button]:min-h-[44px] [&_button]:min-w-[44px]">
						<LanguageSwitch
							className="gap-2"
							onAfterLocaleChange={() => onClose('language_change')}
						/>
					</div>
				</div>
			</aside>
